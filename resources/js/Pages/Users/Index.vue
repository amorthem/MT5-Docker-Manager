...
const canManageUsers = computed(() =>
    ["admin", "dev"].includes(page.props.auth.user.role),
);
...
<template>
    <ManagerLayout title="User Management">
        ...
        <div class="page-heading">
            <div>
                <div class="eyebrow">Access control / directory</div>
                <h1>User management</h1>
                <p>จัดการบัญชีและ role ของทีมผ่าน web controller และ Inertia</p>
            </div>
            <button
                v-if="canManageUsers"
        ...
        <section class="content-panel user-panel">
            <div class="panel-header">
                <div>
                    <h2>Team directory</h2>
                    <p>{{ users.total }} accounts</p>
                </div>
            </div>
            ...
            <div v-else class="table-wrap">
                <table class="container-table user-table">
                    <thead>
                        <tr>
                            <th>USER</th>
                            <th>EMAIL</th>
        ...
        >
            <form class="dialog-form" @submit.prevent="saveUser">
                <div class="modal-kicker">ACCESS / ROLE</div>
                <h2>{{ editingUser ? "แก้ไขผู้ใช้" : "เพิ่มผู้ใช้" }}</h2>
                <p>สิทธิ์จะถูกตรวจสอบซ้ำที่ web controller ทุกครั้ง</p>
                <label
                    >ชื่อผู้ใช้<input v-model="form.name" required /><small
                ...
                ><label
                    >Role<select v-model="form.role">
                        ...
                        <option v-if="isDev" value="admin">admin</option>
                ...
                <div class="modal-actions">
                    ...
                    >
                        {{
                            form.processing ? "กำลังบันทึก..." : "บันทึกผู้ใช้"
                        }}
                    </button>
                </div>
            </form>
